    ...
    try {
        ...
        if (imageBase64) {
            try {
                ...
                console.error('[AddInventory legacy] Cloudinary upload error:', uploadErr);
        ...
        const docRef = await db.collection('inventory').add(newInventory);
        res.status(200).json({ message: 'Inventory added successfully', id: docRef.id, data: newInventory });
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
};
